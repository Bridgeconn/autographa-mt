...
export default function OccurancePanel(props) {
    ...
    const { currentOccurance, tokenDetail, projectId } = useContext(OccuranceContext)
    const [occurance, setOccurance] = React.useState(props.occurance);

    useEffect(() => {
        if (tokenDetail.occurrences.length > 0) {
            let occuranceList = '';
            (tokenDetail.occurrences).map((i) => {
                occuranceList = occuranceList + `sentence_id_list=${i.sentenceId}` + '&'
            })
            API.get(
    ...
    const DisplayOccurance = () => {
        if (occurance.length > 0) {
            return (
                (tokenDetail.occurrences).map((i, k) => {
                    const index = occurance.findIndex(item => item.sentenceId === i.sentenceId);
                    const occ = occurance[index];
                    const sourceOffset = i.offset;
                    const offsetIndex = occ.draftMeta.findIndex(item => item[0][0] === sourceOffset[0]);
                    ...
                    return (
                        <Grid container direction='row' key={k} style={{ backgroundColor: k === currentOccurance - 1 ? '#dde2eb' : '' }}>
                            ...
                            <Grid item md={12} className={classes.sentence}>
                                {offsetIndex === -1 ? occ.draft : <span>{displayText[0]} <b> {displayText[1]}</b> {displayText[2]}</span>}
        ...
        <Paper elevation={3} className={classes.Paper}>
            <Grid className={classes.root} container direction='row'>
                ...
                <Grid item md={3} className={classes.center}>
                    {currentOccurance ? currentOccurance : '0'}/{tokenDetail ? tokenDetail.occurrences.length : '0'}
                </Grid>
                <Grid item md={12}>
